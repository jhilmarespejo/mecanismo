<div class="accordion" id="accordion_observaciones">
    ...
    {{-- se verifica si el array de recomendaciones tiene elementos --}}
    @if (count($a)>0)
         @foreach ( $a as $k=>$reco )
            <div class="accordion-reco bg-primary mt-2">
                <h2 class="accordion-header " id="heading_{{ $reco['REC_id'] }}">
                    <button class="accordion-button collapsed bg-primary text-light" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_{{ $reco['REC_id'] }}" aria-expanded="false" aria-controls="collapse_{{ $reco['REC_id'] }}">
                        <strong>{{ count($a)-$k}}.</strong>&nbsp; {{ substr($reco['REC_recomendacion'], 0, 25) }}... </span>
                    </button>
                </h2>
                <div id="collapse_{{ $reco['REC_id'] }}" class="accordion-collapse collapse  ms-2" aria-labelledby="heading_{{ $reco['REC_id'] }}" data-bs-parent="#accordion_observaciones">
                    <div class="accordion-body bg-light">
                        <dl>
                            <dt>Recomendación:</dt>
                            <dd class="ps-4"> {{ $reco['REC_recomendacion'] }}</dd>

                            <dt>Fecha de la recomendación:</dt>
                            <dd class="ps-4 "> {{ $reco['REC_fechaRecomendacion'] }} </dd>

                            <dt>Tipo de recomendación:</dt>
                            <dd class="ps-4 "> {{ $reco['REC_tipo'] }}</dd>

                            <dt>Cumplimiento de la recomendación:</dt>
                            <dd class="ps-4 pt-3">
                                {{-- {{ ($reco['REC_cumplimiento'] == 0)? 'No' : ' ' }} --}}
                                ...
                                @if ( $reco['REC_cumplimiento'] == 0 || $reco['REC_cumplimiento'] == 2 )
                                    <p class="p-2 mt-2 fs-6 btn bg-primary text-light text-shadow" data-bs-target="#modal_cumplimiento" data-bs-toggle="modal" title="Marcar cumplimiento" onclick="agregarCumplimiento('{{$reco['REC_recomendacion']}}', '{{$reco['REC_fechaRecomendacion']}}', '{{$reco['REC_id']}}')">
                            ...
                            <dd class="ps-4 align-middle">
                                @if ($reco['ARC_id'] != null)
                                    @include('includes.archivos', ['archivos' => $archivosRec, 'id' =>  $reco['REC_id'] ])
                                @endif
                            </dd>
                            <hr>
                            <dt>Fecha de cumplimiento a la Recomendación</dt>
                            <dd class="ps-4">{{ $reco['REC_fechaCumplimiento'] }}</dd>
                            <dt>Detalles del cumplimiento</dt>
                            <dd class="ps-4">{{ $reco['REC_detallesCumplimiento'] }}</dd>

                            <dt>Adjuntos</dt>
                            <dd class="ps-4">
                                @if ($reco['ARC_id'] != null && $reco['REC_detallesCumplimiento'] != null )
                                    @include('includes.archivos', [ 'archivos' => $archivosRecAcato, 'id' =>  $reco['REC_id'] ])
                                @endif
                            </dd>
                        </dl>
                    </div>
